...
class Lirme(Pointwise):
    ...
    #Defined in Pointwise parent class
    ...
    def correctness(self, doc_id, exp_term_vec, rel_doc_vector):
        '''
	Correctness measures: entropy and cosine similarity
        exp_term_vec: list of tuples of form (term, exp_score, tfidf_score)
	rel_doc_vector: dict of {term:tfidf score} generated from the relevant docs for the given query
        '''
	...
	"""
 	word_list: list of all words in the doc
  	explain_tuples: list of explanation tuples generated by Lirme
 	doc_vector: dict {term:tfidf score} of all terms in the doc
  	
 	returns: dict of (word, tfids) of words selected in explanation (sorted order)
  	"""
    ...
    def get_explanation(self, query, doc_id, doc_score, samples, sample_scores, kernel_range, document_vectors, ranker_explanation, num_top_terms, name):
        """
        returns: (explanation_vectors, ranked_lists)
	    explanation_vectors: list of dict of explanation object for different kernel_range values
     	      multiple things stored in the dict. 
	      explanation_vector['term_vector'] contains the explantion term vector 
  	    ranked_lists: list of ranked order of words according to the explanation scores for different kernel_range values
        """
        explanation_vectors = []
        consistency_scores = []
	...
	"""
 	wrapper function to generate Lirme explanation. Calls get_explanation method to generate explanations.
    ...
    def find_relevant_docs(self, query_id, num_rel_docs=10, dataset_name = "msmarco-passage/trec-dl-hard", relevance_label = 1):
        '''
	Find relevant docs for the given query_id using ir_datasets. 
 	Returns rel_docs_list : list of relevant doc_ids with the given relevance_label
        '''
        dataset = ir_datasets.load(dataset_name)
        
        rel_docs_list = []
        for qrel in dataset.qrels_iter():
            if(qrel.query_id  == query_id and qrel.relevance == relevance_label):
                #print(qrel)
    ...
    def generate_ground_truth_terms(self, relevant_doc_list=None):
        '''
	Compute relevant doc vectors from the relevant docs: terms with their normalized tfidf values across all rel docs.
        relevant_doc_list: list of rel doc-ids for the given query

        Returns rel_doc_vectors: dict of form {term:score}  
        '''
        if(relevant_doc_list is None):
            #how to get query_id (and other params for this function)? Need to handle this well!
